...
const Getpost = () => {
  ...
  const GetThePost = async () => {
    await axios
      // .get('http://172.30.1.11:8081/posts?type=diary&year=2022', {
      //   withCredentials: true,
      // })
      .get('http://172.30.1.11:8081/posts/11', {
        withCredentials: true,
      })
      .then(res => {
  ...
  const MapContainer = () => {
    ...
    useEffect(() => {
      ...
      const options = {
        center: new kakao.maps.LatLng(postInfo.lng, postInfo.lat),
        level: 3,
      }
      const map = new kakao.maps.Map(container, options)

      var markerPosition = new kakao.maps.LatLng(postInfo.lng, postInfo.lat)
